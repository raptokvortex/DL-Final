...
def add_distribution_statistics(training_data):
    ...
    # Identifying stock
    # Adding stock label
    training_data['stock_label'] = (training_data['overall mean'] > 550000).astype(int)
...
def drop_stuff(training_data):
    ...
    # Just drop limit order level 2
    if False:
        training_data = training_data.drop('limit order level 2 bid volume', axis = 1)
        training_data = training_data.drop('limit order level 3 bid volume', axis = 1)
        training_data = training_data.drop('limit order level 4 bid volume', axis = 1)

        training_data = training_data.drop('limit order level 2 ask volume', axis = 1)
        training_data = training_data.drop('limit order level 3 ask volume', axis = 1)
        training_data = training_data.drop('limit order level 4 ask volume', axis = 1)

        training_data = training_data.drop('limit order level 2 bid price', axis = 1)
        training_data = training_data.drop('limit order level 3 bid price', axis = 1)
        training_data = training_data.drop('limit order level 4 bid price', axis = 1)

        training_data = training_data.drop('limit order level 2 ask price', axis = 1)
        training_data = training_data.drop('limit order level 3 ask price', axis = 1)
        training_data = training_data.drop('limit order level 4 ask price', axis = 1)
